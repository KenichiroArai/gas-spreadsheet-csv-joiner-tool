...
function combineCsvFromSheet() {
  ...
  writeLog(
    `出力ファイル名：[${outputFile.getName()}], ID：[${outputFile.getId()}]`
  );
...
function writeLog(message: string) {
  ...
  const logSheet: GoogleAppsScript.Spreadsheet.Sheet =
...
function getItemIdFromPath(
  ...
  for (let pathItem of pathItems) {
    ...
    // 現在のフォルダ内のサブフォルダを確認
    let folders: FolderIterator = DriveApp.getFolderById(
      targetFolder.getId()
    ).getFolders();
    ...
    if (targetId == undefined) {
      ...
      let files: FileIterator = DriveApp.getFolderById(
        targetFolder.getId()
      ).getFiles();
      while (files.hasNext()) {
    ...
    if (targetId === undefined) {
      ...
      if (createFile) {
        ...
        const newFile: File = mimeType
          ? targetFolder.createFile(pathItem, "", mimeType)
          : targetFolder.createFile(pathItem, "");
        // 新規ファイルのIDを取得
        targetId = newFile.getId();
        result = targetId;
        writeLog(
          `新規ファイル:[${newFile.getName()}], ID:[${targetId}]を作成しました。`
        );
      }
      return result;
    }
 ...
 */
function getFolderIdFromUrl(url: string) {
 ...
 * - 値が文字列でない場合は "Unknown"
 */
function determineFileInfo(value: string) {
